...
- **`read_recent_posts(host_id)`** — read a host's 20 most recent posts, newest first. Use it after an `instruct_post` timeout to check whether the post was published; do not blindly send the instruction again.
...
- If a chat or post request times out, its result is unknown: check `read_chat_history` or `read_recent_posts` before sending another POST.
